...
def interpolation(x_test, y_test, sample_index, x_baseline, model):
    ...
    inputs = [torch.from_numpy(x.reshape(1,3,500)).type(torch.FloatTensor) for x in sample_values]
    sample_predictions = [model.get_X_preds(x)[0].detach().numpy()[0][sample_label]for x in inputs]
...
def evaluate_faithfulness(n_features, ts_length, x_test, sample_index, grads, net):
	...
	for obs in df_sorted_values[::-1].values:
	    ...
	    perturbed_predictions.append(net.get_X_preds(x_perturbed_torch)[0].detach().numpy())
